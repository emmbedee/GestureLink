...
class GestureLinkApp:
    def __init__(self, window, window_title):
        ...
        self.window.geometry('800x600')  # You can adjust the size as needed
        # Set the minimum size to the initial size (or any size you prefer)
        self.window.minsize(800, 600)  # Adjust minimum size as needed

        style = ttk.Style()
        style.configure('Large.TNotebook.Tab', font=('Helvetica', '12'), padding=[20, 8])
        style.configure('TButton', font=('Helvetica', '10'), background='#e0e0e0')
        style.map('TButton', background=[('active', '#bfbfbf')])

        # Custom styling for the active tab to make it more prominent
        style.map('Large.TNotebook.Tab', background=[('selected', '#bfbfbf')], foreground=[('selected', '#000000')])

        self.tabControl = ttk.Notebook(self.window, style='Large.TNotebook')

        # Actual tabs
        self.tabWebcam = ttk.Frame(self.tabControl, style='BG.TFrame')
        self.tabGestures = ttk.Frame(self.tabControl, style='BG.TFrame')

        # Number of dummy tabs before and after actual tabs
        num_dummy_tabs = 6

        # Add dummy tabs for centering at the beginning
        for _ in range(num_dummy_tabs):
            dummy_tab = ttk.Frame(self.tabControl)
            self.tabControl.add(dummy_tab)
            self.tabControl.tab(dummy_tab, state='disabled')

        # Add actual tabs
        self.tabControl.add(self.tabWebcam, text='Webcam')
        self.tabControl.add(self.tabGestures, text='Gestures')

        # Add dummy tabs for centering at the end
        for _ in range(num_dummy_tabs):
            dummy_tab = ttk.Frame(self.tabControl)
            self.tabControl.add(dummy_tab)
            self.tabControl.tab(dummy_tab, state='disabled')
        ...
        self.canvas = tk.Canvas(self.tabWebcam, bg='#f0f0f0', width=self.vid.get(cv2.CAP_PROP_FRAME_WIDTH), height=self.vid.get(cv2.CAP_PROP_FRAME_HEIGHT))
        self.canvas.pack()

        self.btn_toggle_webcam = tk.Button(self.tabWebcam, text="Start", width=12, height=2, command=self.toggle_webcam, bg='#4caf50', fg='white')
        self.btn_toggle_webcam.pack(pady=20)
        self.btn_toggle_webcam.bind("<Enter>", lambda e: e.widget.config(bg='#45a049'))
        self.btn_toggle_webcam.bind("<Leave>", lambda e: e.widget.config(bg='#4caf50'))
    ...
    def toggle_webcam(self):
        if self.running:
            self.running = False
            self.btn_toggle_webcam.config(text="Start", bg='#4caf50', fg='white')
            self.canvas.delete("all")
        else:
            self.running = True
            self.btn_toggle_webcam.config(text="Stop", bg='#f44336', fg='white')
